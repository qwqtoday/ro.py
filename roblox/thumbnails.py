...
class ThumbnailBatch:
    def __init__(self, token: str, size: str, format: ThumbnailFormat, type: ThumbnailBatchType, requestId: str, targetId: int, alias: str, isCircular: bool):
...
class ThumbnailProvider:
    ...
    async def get_batch_thumbnails(self, batches: list[ThumbnailBatch]) -> list[Thumbnail]:
        thumbnail_response = await self._client.requests.post(
            url=self._client.url_generator.get_url("thumbnails", "/v1/batch"),
            json=[
                {
                    ...
                    "size": _to_size_string(batch.size),
                    "format": batch.format.value,
                    "isCircular": batch.isCircular
                } for batch in batches
            ]
        )
        thumbnails_data = thumbnail_response.json()["data"]
        return [Thumbnail(client=self._client, data=thumbnail_data) for thumbnail_data in thumbnails_data]
